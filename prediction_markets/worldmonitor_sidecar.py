...
class WorldMonitorSidecarHealthSnapshot(BaseModel):
    schema_version: str = "v1"
    sidecar_name: str = "worldmonitor"
    healthy: bool = False
...
class WorldMonitorSidecarBundle(BaseModel):
    schema_version: str = "v1"
    sidecar_name: str = "worldmonitor"
    market_id: str
    ...
    clusters: list["WorldMonitorSidecarClusterSummary"] = Field(default_factory=list)
    linkage: "WorldMonitorSidecarLinkageSummary" = Field(default_factory=lambda: WorldMonitorSidecarLinkageSummary())
    deep_artifacts: dict[str, Any] = Field(default_factory=dict)
    health: WorldMonitorSidecarHealthSnapshot = Field(default_factory=WorldMonitorSidecarHealthSnapshot)
    ...
    @classmethod
    def load(cls, path: str | Path) -> "WorldMonitorSidecarBundle":
...
class WorldMonitorSidecarClusterSummary(BaseModel):
    schema_version: str = "v1"
    cluster_id: str
    label: str
...
class WorldMonitorSidecarLinkageSummary(BaseModel):
    schema_version: str = "v1"
    linkage_id: str = ""
    market_refs: list[str] = Field(default_factory=list)
    event_refs: list[str] = Field(default_factory=list)
...
@dataclass
class WorldMonitorSidecarBridge:
    sidecar_name: str = "worldmonitor"
    default_source_kind: SourceKind = SourceKind.news
    ...
    def ingest(
        ...
        *,
        ...
        inferred_market_id = market_id or _infer_market_id(payload, records) or "worldmonitor"
        ...
        bundle = WorldMonitorSidecarBundle(
            market_id=inferred_market_id,
            venue=venue,
            run_id=run_id,
            source_path=str(source_path) if source_path is not None else None,
    ...
    def _record_to_finding(
        ...
        *,
        ...
        if isinstance(payload, Mapping):
            ...
            if record_id:
                provenance_refs.append(f"record:{record_id}")
        ...
        metadata["cluster_key"] = _cluster_key_from_parts(
            ...
            _strip_or_none(payload.get("layer")) or (finding.tags[0] if finding.tags else "general"),
            topic_hints[0] if topic_hints else (finding.tags[0] if finding.tags else "general"),
        )
        finding.metadata = metadata
        tag_values: list[Any] = list(finding.tags)
        for key in ("tags", "topics", "categories", "layers"):
            tag_values.extend(_string_list(payload.get(key)))
        finding.tags = _flatten_strings(tag_values)
        finding.source_url = finding.source_url or _first_text(payload, ("source_url", "url", "link", "permalink"))
...
def _load_payload_from_path(path: Path) -> tuple[Any, SidecarPayloadKind, Path | None, list[str]]:
    ...
    payload, payload_kind, errors = _parse_text_payload(text, source_path=path)
...
def _load_payload_from_text(text: str) -> tuple[Any, SidecarPayloadKind, Path | None, list[str]]:
    payload, payload_kind, errors = _parse_text_payload(text, source_path=None)
    return payload, payload_kind, None, errors


def _parse_text_payload(text: str, *, source_path: Path | None) -> tuple[Any, SidecarPayloadKind, list[str]]:
    stripped = text.strip()
    if not stripped:
...
def _extract_records(payload: Any) -> list[Any]:
    ...
    if isinstance(payload, Mapping):
        records: list[Any] = []
        for key in ("records", "items", "events", "signals", "articles", "stories", "updates", "posts", "messages", "data"):
...
def _source_kind_from_record(record: Mapping[str, Any] | str, *, default: SourceKind) -> SourceKind:
    if isinstance(record, str):
        text = record.lower()
        if any(token in text for token in ("twitter", "x.com", "reddit", "post", "comment")):
            return SourceKind.social
        return default
    raw = record.get("source_kind") or record.get("kind") or record.get("layer") or record.get("source")
    if raw is None:
        return default
...
def _default_provenance_refs(source_path: Path | None, market_id: str, run_id: str | None) -> list[str]:
    refs = [f"sidecar:worldmonitor", f"market:{market_id}"]
    if source_path is not None:
...
def _bundle_content_hash(bundle: WorldMonitorSidecarBundle) -> str:
...
def _build_health(
    *,
    ...
    if parsed_count <= 0:
        ...
        healthy = True
        message = "healthy"
    return WorldMonitorSidecarHealthSnapshot(
...
def _record_fingerprint(payload: Mapping[str, Any]) -> str:
    fingerprint_payload = {
        ...
        "title": _first_text(payload, ("title", "headline")),
        "summary": _first_text(payload, ("summary", "message", "note", "body", "text", "content")),
        "source_url": _first_text(payload, ("source_url", "url", "link", "permalink")),
        "source_kind": _strip_or_none(payload.get("source_kind") or payload.get("kind") or payload.get("layer") or payload.get("source")),
        "published_at": _first_text(payload, ("published_at", "created_at", "timestamp", "observed_at")),
...
def _build_deep_artifacts(
    *,
    ...
    clusters: list[WorldMonitorSidecarClusterSummary] = []
    ...
    for key, indexes in sorted(cluster_map.items()):
        ...
        layer = _cluster_layer(cluster_findings)
        region = _cluster_region(cluster_findings)
        topic = _cluster_topic(cluster_findings)
        source_kind = _cluster_source_kind(cluster_findings)
        cluster_id = _stable_ref("worldmonitor_cluster", market_id, run_id, key)
        ...
        market_refs = _dedupe([f"market:{market_id}", *_refs_from_findings(cluster_findings, "market_refs"), *_market_refs_from_findings(cluster_findings)])
        ...
        clusters.append(
            WorldMonitorSidecarClusterSummary(
                cluster_id=cluster_id,
                label=f"{source_kind}:{layer}:{region}:{topic}",
                source_kind=source_kind,
                layer=layer,
                region=region,
                topic=topic,
    ...
    market_refs = _dedupe([f"market:{market_id}", *_refs_from_findings(findings, "market_refs"), *_market_refs_from_findings(findings)])
    ...
    cluster_refs = [cluster.cluster_id for cluster in clusters]
    cluster_labels = [cluster.label for cluster in clusters]
    linked_pairs = _dedupe([f"{market_ref}|{event_ref}" for market_ref in market_refs for event_ref in event_refs] or [f"market:{market_id}"])
    linkage_id = _stable_ref("worldmonitor_linkage", market_id, run_id, *cluster_refs, *market_refs, *event_refs)
    provenance_refs = _dedupe(
        [
    ...
    linkage = WorldMonitorSidecarLinkageSummary(
...
def _extract_market_refs(payload: Mapping[str, Any], market_id: str) -> list[str]:
    ...
    for key in ("market_id", "market_ids", "market_slug", "market", "market_ref", "ticker", "symbol"):
...
def _extract_event_refs(payload: Mapping[str, Any]) -> list[str]:
    refs: list[str] = []
    for key in ("event_id", "event_ids", "canonical_event_id", "signal_id", "article_id", "story_id"):
...
def _extract_question_hints(payload: Mapping[str, Any]) -> list[str]:
    hints: list[str] = []
    for key in ("question", "question_hint", "prompt", "title", "headline", "summary"):
...
def _extract_topic_hints(payload: Mapping[str, Any]) -> list[str]:
    hints: list[str] = []
    for key in ("topic", "theme", "category", "cluster", "tags", "topics", "layers", "regions", "country"):
...
def _cluster_key_for_finding(finding: ResearchFinding) -> str:
    return _cluster_key_from_parts(
        finding.source_kind.value,
        _strip_or_none(finding.metadata.get("layer")) or (finding.tags[0] if finding.tags else "general"),
        _strip_or_none(finding.metadata.get("region")) or _strip_or_none(finding.metadata.get("country")) or "global",
        _cluster_topic([finding]),
    )
...
def _cluster_layer(findings: Sequence[ResearchFinding]) -> str:
    for finding in findings:
        layer = _strip_or_none(finding.metadata.get("layer"))
        if layer:
            return _normalize_topic(layer)
    return "general"


def _cluster_region(findings: Sequence[ResearchFinding]) -> str:
    for finding in findings:
        region = _strip_or_none(finding.metadata.get("region")) or _strip_or_none(finding.metadata.get("country"))
        if region:
            return _normalize_topic(region)
    return "global"
