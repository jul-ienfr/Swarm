...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const payload = await advisePredictionMarket({
      ...result.data,
      request_mode: result.data.request_mode ?? 'predict',
      response_variant: result.data.response_variant ?? 'standard',
    ...
    logger.error({ err: error }, 'POST /api/v1/prediction-markets/predict error')
    const response = toPredictionMarketsErrorResponse(error, 'Failed to run prediction-markets predict')
