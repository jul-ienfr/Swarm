...
const mocks = vi.hoisted(() => ({
  requireRole: vi.fn(),
  heavyLimiter: vi.fn(),
  preparePredictionMarketRunPaper: vi.fn(),
}))
...
vi.mock('@/lib/prediction-markets/service', () => ({
  preparePredictionMarketRunPaper: mocks.preparePredictionMarketRunPaper,
}))

describe('prediction markets v1 paper route', () => {
  beforeEach(() => {
    vi.resetModules()
    mocks.requireRole.mockReset()
    mocks.heavyLimiter.mockReset()
    mocks.preparePredictionMarketRunPaper.mockReset()
    mocks.requireRole.mockReturnValue({ user: { workspace_id: 7, username: 'operator' } })
    mocks.heavyLimiter.mockReturnValue(null)
  })

  it('returns a paper surface payload with v1 header', async () => {
    mocks.preparePredictionMarketRunPaper.mockReturnValue({
      ...
      source_refs: {
        run_detail: 'run-paper-1',
        execution_projection: 'run-paper-1:execution_projection',
        paper_projected_path: 'run-paper-1:execution_projection#paper',
        trade_intent_guard: 'run-paper-1:trade_intent_guard',
        multi_venue_execution: 'run-paper-1:multi_venue_execution',
      },
      paper_path: {
        path: 'paper',
        status: 'ready',
    ...
    const { POST } = await import('@/app/api/v1/prediction-markets/runs/[run_id]/paper/route')
    const request = new NextRequest('http://localhost/api/v1/prediction-markets/runs/run-paper-1/paper', {
      method: 'POST',
    })

    const response = await POST(request, {
      params: Promise.resolve({ run_id: 'run-paper-1' }),
    })
    ...
    expect(mocks.preparePredictionMarketRunPaper).toHaveBeenCalledWith({
      runId: 'run-paper-1',
      workspaceId: 7,
    })
  ...
  it('preserves paper blockers when benchmark promotion is not ready', async () => {
    mocks.preparePredictionMarketRunPaper.mockReturnValue({
      gate_name: 'execution_projection_paper',
      preflight_only: true,
      run_id: 'run-paper-blocked-1',
      workspace_id: 7,
      surface_mode: 'paper',
      paper_status: 'blocked',
      paper_blocking_reasons: ['paper_path_unavailable', 'research_benchmark:out_of_sample_unproven'],
      summary: 'Paper surface is blocked by missing path and benchmark promotion.',
      source_refs: {
        run_detail: 'run-paper-blocked-1',
        execution_projection: 'run-paper-blocked-1:execution_projection',
        paper_projected_path: null,
        trade_intent_guard: null,
        multi_venue_execution: null,
      },
      paper_path: null,
      paper_trade_intent_preview: null,
      paper_trade_intent_preview_source: null,
      research_benchmark_gate_summary:
    ...
    const { POST } = await import('@/app/api/v1/prediction-markets/runs/[run_id]/paper/route')
    const request = new NextRequest('http://localhost/api/v1/prediction-markets/runs/run-paper-blocked-1/paper', {
      method: 'POST',
    })

    const response = await POST(request, {
      params: Promise.resolve({ run_id: 'run-paper-blocked-1' }),
    })
    const body = await response.json()
  ...
  it('returns the limiter response before calling the paper service', async () => {
    mocks.heavyLimiter.mockReturnValue(
      NextResponse.json({ error: 'Too Many Requests' }, { status: 429 }),
    )

    const { POST } = await import('@/app/api/v1/prediction-markets/runs/[run_id]/paper/route')
    const request = new NextRequest('http://localhost/api/v1/prediction-markets/runs/run-paper-2/paper', {
      method: 'POST',
    })

    const response = await POST(request, {
      params: Promise.resolve({ run_id: 'run-paper-2' }),
    })
    const body = await response.json()

    expect(response.status).toBe(429)
    expect(body).toEqual({ error: 'Too Many Requests' })
    expect(mocks.preparePredictionMarketRunPaper).not.toHaveBeenCalled()
  })
  ...
  it('short-circuits on auth errors before rate limiting or service calls', async () => {
    ...
    const { POST } = await import('@/app/api/v1/prediction-markets/runs/[run_id]/paper/route')
    const request = new NextRequest('http://localhost/api/v1/prediction-markets/runs/run-paper-3/paper', {
      method: 'POST',
    })

    const response = await POST(request, {
      params: Promise.resolve({ run_id: 'run-paper-3' }),
    })
    const body = await response.json()
  ...
  it('maps service exceptions through the prediction markets error formatter', async () => {
    mocks.preparePredictionMarketRunPaper.mockImplementation(() => {
      throw new Error('paper exploded')
    })

    const { POST } = await import('@/app/api/v1/prediction-markets/runs/[run_id]/paper/route')
    const request = new NextRequest('http://localhost/api/v1/prediction-markets/runs/run-paper-4/paper', {
      method: 'POST',
    })

    const response = await POST(request, {
      params: Promise.resolve({ run_id: 'run-paper-4' }),
    })
    const body = await response.json()

    expect(response.status).toBe(500)
    expect(body).toEqual({
      error: 'paper exploded',
      code: 'internal_error',
    })
  })
})
