...
describe('prediction markets CLI shadow', () => {
  ...
  beforeAll(async () => {
    server = createServer((req, res) => {
      ...
      if (req.method === 'POST' && url.pathname === '/api/v1/prediction-markets/runs/run-shadow-001/shadow') {
        ...
        res.end(JSON.stringify({
          ...
          ...makeResearchHints(),
          source_refs: {
            run_detail: 'run-shadow-001',
            execution_projection: 'run-shadow-001:execution_projection',
            shadow_projected_path: 'run-shadow-001:execution_projection#shadow',
            shadow_arbitrage: 'run-shadow-001:shadow_arbitrage',
            trade_intent_guard: 'run-shadow-001:trade_intent_guard',
            multi_venue_execution: 'run-shadow-001:multi_venue_execution',
          },
          shadow_path: {
            path: 'shadow',
          ...
          execution_projection_preflight_summary: {
            ...
            source_refs: ['run-shadow-001:pipeline_guard'],
            blockers: [],
  ...
  it('calls the shadow route and prints the compact shadow surface summary', async () => {
    ...
    expect(result.stdout).toContain('shadow_surface: status=ready')
    expect(result.stdout).toContain('shadow_surface: status=ready gate=execution_projection_shadow preflight=yes run_id=run-shadow-001 requested=live path_status=ready effective_mode=shadow selected=shadow research_mode=research_driven research_origin=research_driven blockers=0 size=60')
  ...
  it('returns shadow_status under data in json mode', async () => {
    const result = await execFileAsync(
      process.execPath,
      [
        CLI,
