...
describe('dashboard contract index coverage', () => {
  it('keeps the dashboard and run-surface routes visible from the API index', async () => {
    const { GET } = await import('@/app/api/index/route')
    const response = await GET()
    const payload = await response.json() as { endpoints: Array<{ path: string; auth: string; method: string }> }
