...
const renderGraph = () => {
  ...
  // Process edge data, compute edge count and index between node pairs
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Compute curvature: spread multiple edges, single edge is straight
    let curvature = 0
    if (totalCount > 1) {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to cluster isolated nodes
  ...
  const getLinkPath = (d) => {
    ...
    // Compute curve control points - dynamic based on edge count and distance
  ...
  // Save ref for external visibility control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position, no simulation restart (distinguish click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('end', (event, d) => {
        // Let simulation cool down only after actual drag
        if (d._isDragging) {
