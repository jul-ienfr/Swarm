...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-queries - complete extraction
    ...
    // Extract key facts - complete extraction
    ...
    // Extract core entities - complete with summary and related fact count
    ...
    // Extract relationship chains - complete extraction
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/(?:当前有效事实|Current Active Facts):\s*(\d+)/)
    ...
    // Extract current active facts - complete extraction
    ...
    // Extract historical/expired facts - complete extraction
    ...
    // Extract involved entities - complete extraction
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### (?:采访对象选择理由|Interviewee Selection Reasons)\n([\s\S]*?)(?=\n---\n|\n### (?:采访实录|Interview Records))/)
    if (reasonMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - complete extraction
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no number found or only one, return as whole
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: first question returns full answer, others empty
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers and content differs
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (show only with real dual-platform answers)
                  ...
                  // Expand/Collapse Button (hide for placeholder text)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 since section title is shown externally
  ...
  // Process line breaks - empty lines become paragraph breaks
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <p><br> before block elements (extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> between block elements and paragraph starts
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If complete, scroll to top; otherwise follow latest log
            if (isComplete.value) {
