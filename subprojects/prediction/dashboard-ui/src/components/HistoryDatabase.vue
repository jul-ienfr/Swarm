      ...
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Prediction playback divider -->
            <div class="modal-divider">
              <span class="divider-line"></span>
              <span class="divider-text">Prediction Playback</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 'Start Simulation' and Step5 'Deep Interaction' must be started during a live run and do not support history playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Compute actual height: rows * card height + gaps + bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute cards per row, center each row
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, just record state for later
        if (isAnimating) return
        
        // Skip if target matches current state
        ...
        // Short delay for expand (50ms), longer for collapse (200ms)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs execution
          ...
          // Unlock after animation, check pending states
          setTimeout(() => {
            isAnimating = false
            
            // After animation, check for new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay slightly before executing to avoid fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin for scroll trigger sensitivity
...
// Watch route changes, reload data on home return
...
// Reload data on keep-alive component activation
onActivated(() => {
  loadHistory()
})
...
.grid-pattern {
  ...
  /* Position from top-left, height changes expand at bottom only */
