  ...
  <div class="polymarket-container">
    <div class="main-content">
      ...
      <section class="header-section">
        ...
        <h1 class="main-title">PolFish Predict</h1>
      ...
      <!-- Active Predictions -->
      <section v-if="activePredictions.length > 0" class="active-predictions-section">
        <div class="section-header">
          <span class="status-dot">&#9632;</span>
          <span class="section-label">Active Predictions</span>
          <span class="history-count">{{ activePredictions.length }}</span>
        </div>
        <div class="predictions-grid">
          <router-link
            v-for="(pred, idx) in activePredictions"
            :key="idx"
            :to="'/predict/' + (pred.slug || pred.market_id)"
            class="prediction-card"
            :class="predictionCardClass(pred)"
          >
            <div class="pred-card-question">{{ pred.question || pred.slug || pred.market_id }}</div>
            <div class="pred-card-row">
              <div class="pred-card-stat">
                <span class="pred-card-label">Market</span>
                <span class="pred-card-value">{{ ((pred.odds || 0) * 100).toFixed(0) }}%</span>
              </div>
              <div class="pred-card-stat">
                <span class="pred-card-label">Ours</span>
                <span class="pred-card-value">{{ pred.prediction ? ((pred.prediction) * 100).toFixed(0) + '%' : '--' }}</span>
              </div>
              <div class="pred-card-stat">
                <span class="pred-card-label">Edge</span>
                <span class="pred-card-value" :class="edgeCardClass(pred)">{{ pred.edge ? (pred.edge > 0 ? '+' : '') + ((pred.edge) * 100).toFixed(1) + '%' : '--' }}</span>
              </div>
              <span class="pred-card-signal" :class="signalClass(pred.signal || 'SKIP')">
                {{ pred.signal || (pred.side === 'YES' ? 'BUY_YES' : pred.side === 'NO' ? 'BUY_NO' : 'SKIP') }}
              </span>
            </div>
            <div class="pred-card-footer">
              <span v-if="pred.mode" class="badge badge-mode" :class="pred.mode === 'deep' ? 'badge-deep' : 'badge-quick'">{{ pred.mode }}</span>
              <span class="pred-card-amount" v-if="pred.amount">${{ pred.amount.toFixed(0) }}</span>
              <span class="pred-card-status" :class="pred.resolved ? 'status-resolved' : 'status-open'">{{ pred.resolved ? 'Resolved' : 'Open' }}</span>
            </div>
          </router-link>
        </div>
      </section>
...
const predictionCardClass = (pred) => {
  const edge = pred.edge || 0
  if (Math.abs(edge) < 0.01) return 'pred-neutral'
  return edge > 0.05 ? 'pred-positive' : edge < -0.05 ? 'pred-negative' : 'pred-neutral'
}

const edgeCardClass = (pred) => {
  const edge = pred.edge || 0
  if (edge > 0.05) return 'edge-positive'
  if (edge < -0.05) return 'edge-negative'
  return ''
}
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (json.success && json.data) {
        const open = json.data.open_positions || []
        const resolved = json.data.resolved_positions || []
        activePredictions.value = [...open, ...resolved.slice(0, 10)]
      }
    }
  } catch { /* ignore */ }
}
...
.prediction-card.pred-positive {
  border-left: 4px solid var(--green);
}

.prediction-card.pred-negative {
  border-left: 4px solid var(--red);
}

.prediction-card.pred-neutral {
