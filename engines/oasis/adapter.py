...
from .translator import OASISTranslator


class OASISEngineAdapter:
    def __init__(self, engine_client, store: RunMappingStore, artifact_base: str = "/tmp/oasis"):
        self._client = engine_client
        self._store = store
        self._translator = OASISTranslator()
        self._monitor = OASISMonitor(engine_client=engine_client)
    ...
    def create_run(self, request: CreateRunRequest) -> CreateRunResponse:
        ...
        try:
            ...
            raise EngineUnavailableError(engine="oasis", detail=str(exc)) from exc
    ...
    def get_status(self, request: GetStatusRequest) -> StatusResponse:
        ...
        if not mapping or not mapping.engine_run_id:
            return self._error_result(request, "No engine_run_id found for runtime run.")
            ...
            if current_status == RunStatus.failed:
                errors.append(
                    NormalizedError.from_code(
                        ...
                        f"OASIS run finished with status '{current_status.value}'.",
                    )
                )
            return AdapterResultV1(
                runtime_run_id=request.runtime_run_id,
    ...
    def _error_result(self, request: GetStatusRequest | GetResultRequest, message: str) -> AdapterResultV1:
        return AdapterResultV1(
            runtime_run_id=request.runtime_run_id,
